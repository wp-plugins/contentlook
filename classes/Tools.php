 ...
 * @author StarBox
 */
class CL_Classes_Tools extends CL_Classes_FrontController {
    ...
    public function hookActionlink($links, $file) {
        ...
        if ($file == strtolower(_CL_PLUGIN_NAME_) . '/' . strtolower(_CL_PLUGIN_NAME_) . '.php') {
            $link = '<a href="' . admin_url('admin.php?page=cl_settings') . '">' . __('Settings', _CL_PLUGIN_NAME_) . '</a>';
    ...
    public static function saveOptions($key, $value) {
        ...
        update_option(CL_OPTION, json_encode(self::$options));
    }
    ...
    public static function setHeader($type) {
        if (CL_Classes_Tools::getValue('cl_debug') == 'on')
            return;
     ...
     * @return void
     */
    public static function showNotices($message, $type = 'cl_notices') {
    ...
    private function loadMultilanguage() {
        if (!defined('WP_PLUGIN_DIR')) {
            load_plugin_textdomain(_CL_PLUGIN_NAME_, _CL_PLUGIN_NAME_ . '/languages/');
        } else {
            load_plugin_textdomain(_CL_PLUGIN_NAME_, null, _CL_PLUGIN_NAME_ . '/languages/');
        }
    }
     ...
     */
    public static function cl_remote_get($url, $param = array()) {
        ...
        if (function_exists('curl_init')) {
            return self::cl_curl($url, array('timeout' => $timeout,));
        } else {
            return self::cl_wpcall($url, array('timeout' => $timeout));
        }
    }
     ...
     * @return string
     */
    private static function cl_curl($url, $param = array()) {
        $ch = curl_init($url);
        ...
        if (curl_errno($ch) == 1) { //if protocol not supported
            ...
            $response = self::cl_wpcall($url, $param); //use the wordpress call
        }
     ...
     * @return string
     */
    private static function cl_wpcall($url, $param = array()) {
     ...
     * Connect remote with CURL if exists
     */
    public static function cl_remote_head($url) {
        $response = array();
    ...
    public static function checkErrorSettings($count_only = false) {
        ...
        if (false) {
            if ($count_only)
                ...
                CL_Classes_Error::setError(__('Notice', _CL_PLUGIN_NAME_) . " <br /> ", 'settings');
        }
    }
    ...
    private function checkDebug() {
        //if debug is called
        if (self::getIsset('cl_debug')) {
            if (self::getValue('cl_debug') === 'on') {
