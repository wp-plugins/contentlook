 ...
 */
class CL_Classes_Action extends CL_Classes_FrontController {
    ...
    function hookHead() {
        ...
        echo '<script type="text/javascript">
                  var cl_Query = {
                    ...
                    "nonce": "' . wp_create_nonce(_CL_NONCE_ID_) . '"
                  }
              </script>';
    }
    ...
    public function getActions($cur_action) {
        ...
        foreach ($this->actions as $actions) {
            CL_Classes_ObjController::getController($actions['class'])->action();
        }
    }

    /**
     * Call the Contentlook Api Server
     * @param string $module
     * @param array $args
     * @return json | string
     */
    public static function apiCall($module, $args = array(), $timeout = 60) {
        ...
        $extra = array('token' => CL_Classes_Tools::$options['cl_token']);
        ...
        $url = self::cleanUrl(_CL_API_URL_ . $module . "?" . $parameters);
        return CL_Classes_Tools::cl_remote_get($url, array('timeout' => $timeout));
    }
