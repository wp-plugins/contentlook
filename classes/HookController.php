 ...
 */
class CL_Classes_HookController {
    ...
    public function getShortcodes() {
        ...
        if (CL_Classes_Tools::getOption('cl_shortcode') == 0)
            return;

        self::$shortCodesSet = true;
        /* if config allready in cache */
        if (!isset(CL_Classes_ObjController::$config)) {
            $config_file = _CL_CORE_DIR_ . 'config.xml';
            if (!file_exists($config_file))
                return;

            /* load configuration blocks data from core config files */
            $data = file_get_contents($config_file);
            CL_Classes_ObjController::$config = json_decode(json_encode((array) simplexml_load_string($data)), 1);
        }
        // echo '<pre>' . print_r(CL_Classes_ObjController::$config['block'], true) . '</br>';
        //print_r(CL_Classes_ObjController::$config);
        if (is_array(CL_Classes_ObjController::$config))
            foreach (CL_Classes_ObjController::$config['block'] as $block) {
                if (isset($block['name'])) {
                    if (isset($block['active']) && $block['active'] == 1)
                        if (isset($block['shortcodes']['shortcode'])) {
                            $instance = CL_Classes_ObjController::getController($block['name']);
